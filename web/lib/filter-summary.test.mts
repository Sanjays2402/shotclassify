...
// --- F91: compact active-filter count pill -------------------------------

test("filterCountLabel: null at zero (no inert '0 filters')", () => {
  assert.equal(filterCountLabel({}), null);
  assert.equal(filterCountLabel({ minConfPct: 0 }), null);
  assert.equal(filterCountLabel({ category: "", q: "  ", pinnedOnly: false }), null);
});

test("filterCountLabel: singular at one, plural beyond", () => {
...
test("filterCountLabel: count always tracks countActiveFilters", () => {
  const f: ShotFilterState = {
    category: "chat",
    q: "ship",
    minConfPct: 50,
  ...
  assert.equal(n, 4); // category + q + minConf + since
  assert.equal(filterCountLabel(f), `${n} filters`);
});
